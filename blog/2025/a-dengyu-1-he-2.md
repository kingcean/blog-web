...
原来，对于对象，我们还可以实现其 `toJSON()` 成员方法，当该对象自身或其作为其它对象的属性，被执行 `JSON.stringif()` 执行序列化时，如果存在，便会自动隐式先调用该方法。该方法需要返回一个值，可以是简单类型（如字符串、布尔值和数字）的值，也可以是对象或数组，其序列化便会是该值。
